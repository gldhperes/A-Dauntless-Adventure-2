...
public class Level_Islands : MonoBehaviour
{
    ...
    public void spawnIsland()
    {
        ...
        // if( game_Events.enemysDefeated >= game_Events.getEnemysToSpawnCheckpoint() ){
    ...
    private void spawnGroundEnemyOnIsland()
    {
        ...
        int maxGroundEnemysinIsland = Random.Range (0, game_Events.getMaxGroundEnemysInIsland());
        ...
        foreach (var pos in tilemap.cellBounds.allPositionsWithin)
        {
            ...
            if (tilemap.HasTile(localPlace))
            {
                ...
                if (tile.name == "Grass_01" || tile.name == "Desert_01")
                {
                    ...
                    {
                        GameObject enemyGround = Instantiate(game_Events.getEnemyGroundPrefab(), place, Quaternion.identity) as GameObject;
